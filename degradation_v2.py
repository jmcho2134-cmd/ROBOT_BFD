...
def predict_degradation_score(model, C_demo, actions, z, phase, d_z, terms, scales,
                              lam, horizon, anchors=None, n_anchor=5):
    """FROZEN FCM 을 surrogate 로 써서 방향 d_z(크기 lam)의 subgoal degradation 예측 점수.
    proposal §9.3: '여러 anchor 와 작은 magnitude 에서 평균'. 높을수록 더 열화.
    step (2)(degradation.optimize_direction_cem)가 이 함수를 objective 로 최대화한다.
    FCM 은 재학습되지 않는다 -- 이미 학습된 예측기를 '환경'처럼 질의만 한다."""
    span = phase_span(z, phase, len(z)) or (0, len(z))
    a_, b_ = span
    if anchors is None:
        if b_ - a_ <= 1:
            anchors = [int(a_)]
        else:                                     # phase 전체에 고르게 anchor (단일 midpoint 아님)
            anchors = np.unique(
                np.linspace(a_, b_ - 1, min(n_anchor, b_ - a_)).astype(int)).tolist()
    D = (float(lam) * np.asarray(d_z, float)).reshape(1, -1)
    scores = []
    for t in anchors:
        zt = float(z[min(t, len(z) - 1)]); rho = phase_progress(z, t)
        c0 = np.asarray(C_demo[t]).reshape(1, -1)
        a0 = np.asarray(actions[t]).reshape(1, -1)
        pr = model.predict(c0, a0, D, float(horizon), zt, rho)
        scores.append(subgoal_score(pr[0], terms, scales))
